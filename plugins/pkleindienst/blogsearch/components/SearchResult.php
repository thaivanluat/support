...
class SearchResult extends ComponentBase
{
    ...
    public function defineProperties()
    {
        ...
        return [
            ...
            'noPostsMessage' => [
                ...
                'type'         => 'string',
                'default'      => 'Rất tiếc! Không tìm thấy dữ liệu liên quan',
                'showExternalParam' => false
            ],
            'sortOrder' => [
    ...
    public function onRun()
    {
        ...
        foreach($this->posts as $post){
            $postSlug = $post->slug;
            $currentCategory = $this->loadCategory($postSlug);
    ...
    protected function loadPost($slug)
    {
        $post = new BlogPost;
    ...
    protected function loadCategory($postSlug)
    {
        //find parent
        if ( Input::get('cslug', false) ) {
            ...
            $post = $this->loadPost($postSlug);
            $categories = Db::table('rainlab_blog_posts')
            ...
            ->where('rainlab_blog_posts.id', $post->id)
            ->get();
